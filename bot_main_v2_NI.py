...
class DirectPage:
    ...
    def login(self, username, password):
        ...
        # Кнопка для прохода (Вроде не надо)
        #sleep(4)
    ...
    def telegram_track(self, app, num):

        path = os.getcwd() + f"\\video\\{num}.ogg"
        app.start()

        app.send_voice("YaMelodyBot", voice=path, duration=30)
        sleep(5)
        text = app.get_history("YaMelodyBot", limit=1)[0]["text"]
    ...
    # ДОРАБОТАТЬ APPLE MUSIC
    def get_applemusic_link(self, music_name):
        # apple music link
        url_apple_music = 'https://music.apple.com/us/search?term='
...
def main():
    # telegram
    app = Client("my_account1", config_file="config.ini", takeout=False)
    ...
    def process_message(dm, app, num, i):
        print(f'---------- N: {i} ----------')
        ...
        if event != 'continue':
            ...
            if video_url is not None:
                ...
                music_data = dm.telegram_track(app, num)
                if len(music_data) == 2:
    ...
    while True:
        ...
        for num in range(num_new):
            process_message(dm, app, num, mess_num)
            mess_num += 1
            print('[INFO] DONE')
            sleep(1)
